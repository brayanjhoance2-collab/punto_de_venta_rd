...
export default function EditarCompra() {
    ...
    const cargarDatos = async () => {
        setCargando(true)
        try {
            const [resultadoCompra, resultadoFormulario] = await Promise.all([
                obtenerCompra(compraId),
                obtenerDatosFormulario()
            ])

            if (resultadoCompra.success && resultadoFormulario.success) {
                const compra = resultadoCompra.compra
                ...
                alert(resultadoCompra.mensaje || resultadoFormulario.mensaje || 'Error al cargar datos')
    ...
    const manejarSubmit = async (e) => {
        ...
        if (!confirm('¿Estas seguro de actualizar esta compra? Los cambios afectaran el inventario.')) {
            return
        }
        ...
        try {
            ...
            if (resultado.success) {
                ...
                alert(resultado.mensaje || 'Error al actualizar compra')
            }
        } catch (error) {
    ...
    return (
        <div className={`${estilos.contenedor} ${estilos[tema]}`}>
            <div className={estilos.header}>
                <div>
                    <h1 className={estilos.titulo}>Editar Compra</h1>
                    <p className={estilos.subtitulo}>Modifica los datos de la compra</p>
                </div>
                <button
                    type="button"
            ...
            <form onSubmit={manejarSubmit} className={estilos.formulario}>
                <div className={estilos.fila}>
                    ...
                    <div className={`${estilos.panel} ${estilos[tema]}`}>
                        ...
                        >
                            {procesando ? 'Actualizando...' : 'Actualizar Compra'}
                        </button>
                    </div>
                </div>
